...
| `story/deflect_arrow` | `minecraft:story/obtain_armor` | `deflected_projectile` | `minecraft:entity_hurt_player` | `damage.type.tags[]`, `damage.blocked` | `[["deflected_projectile"]]` | done | done | Not `killed_by_arrow`. 已落地窄实现：仅支持 `damage.blocked = true` + `minecraft:is_projectile` 这组已核 surface。 |
| `adventure/sniper_duel` | `minecraft:adventure/shoot_arrow` | `killed_skeleton` | `minecraft:player_killed_entity` | `entity[]`, `entity[].condition`, `entity[].entity`, `entity[].predicate.type`, `entity[].predicate.distance.horizontal.min`, `killing_blow.tags[]` | `[["killed_skeleton"]]` | done | done | Not `killed_by_arrow`. 已落地窄实现：仅支持已核 shape（skeleton + 水平距离 `>= 50.0` + projectile killing_blow tags）。 |
...
- `story/deflect_arrow` is now shipped locally with JSON and both language keys.
- `adventure/sniper_duel` is now shipped locally with JSON and both language keys.
- Current compiled trigger-condition support now includes the shipped narrow surfaces for:
  ...
  - `minecraft:entity_hurt_player` via `damage.blocked = true` + `minecraft:is_projectile`
- Of the verified trigger IDs in this wave, `minecraft:shot_crossbow`, `minecraft:player_hurt_entity`, `minecraft:player_killed_entity` (including the sniper_duel-only nested slice), the bullseye-only slice of `minecraft:target_hit`, and the deflect-arrow-only slice of `minecraft:entity_hurt_player` are now wired at runtime.
