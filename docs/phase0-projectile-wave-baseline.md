...
- `adventure/sniper_duel` still has no local advancement JSON row.
...
- Of the verified trigger IDs in this wave, `minecraft:shot_crossbow`, `minecraft:player_hurt_entity`, `minecraft:player_killed_entity`, the bullseye-only slice of `minecraft:target_hit`, and the deflect-arrow-only slice of `minecraft:entity_hurt_player` are now wired at runtime.
